...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`consulting-firm:${ip}`, 8, 60_000)) {
  ...
  try {
    ...
    await Promise.resolve({
      route: 'consulting-firm',
      framework: 'Consulting Firm',
...
export async function GET() {
  return NextResponse.json({ status: 'healthy', service: 'Consulting Firm', version: '1.0.0' });
}
