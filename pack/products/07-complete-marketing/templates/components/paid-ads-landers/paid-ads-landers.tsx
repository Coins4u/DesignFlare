...
export interface PaidAdsLandersProps {
  brandName?: string;
...
/** Paid Ads Landers — Enterprise white-label module · Complete Marketing */
export function PaidAdsLandersBlock({ brandName = 'Your Agency', accent = 'indigo', onDeploy }: PaidAdsLandersProps) {
    ...
    <section className={`overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl ${colors.ring} ring-1`}>
      <header className="border-b border-slate-100 bg-slate-50/80 px-8 py-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className={`inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ${colors.badge}`}>Enterprise · Paid Ads Landers</p>
            <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-slate-900">{brandName} — Paid Ads Landers</h2>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {tab === 'config' && (
            <div className="rounded-2xl border border-slate-200 p-5 font-mono text-xs text-slate-700">
              <pre>{JSON.stringify({ module: 'Paid Ads Landers', license: 'Complete Marketing', whiteLabel: true, tokens: ['primary', 'surface', 'radius'] }, null, 2)}</pre>
            </div>
          )}
          {tab === 'analytics' && (
