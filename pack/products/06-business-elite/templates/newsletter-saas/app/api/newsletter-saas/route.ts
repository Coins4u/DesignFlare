...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`newsletter-saas:${ip}`, 8, 60_000)) {
  ...
  try {
    ...
    await Promise.resolve({
      route: 'newsletter-saas',
      framework: 'Newsletter SaaS',
...
export async function GET() {
  return NextResponse.json({ status: 'healthy', service: 'Newsletter SaaS', version: '1.0.0' });
}
