...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`webinar-funnel:${ip}`, 8, 60_000)) {
  ...
  try {
    ...
    await Promise.resolve({
      route: 'webinar-funnel',
      framework: 'Webinar Funnel',
...
export async function GET() {
  return NextResponse.json({ status: 'healthy', service: 'Webinar Funnel', version: '1.0.0' });
}
