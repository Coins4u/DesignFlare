...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`membership-site:${ip}`, 8, 60_000)) {
  ...
  try {
    ...
    await Promise.resolve({
      route: 'membership-site',
      framework: 'Membership Site',
...
export async function GET() {
  return NextResponse.json({ status: 'healthy', service: 'Membership Site', version: '1.0.0' });
}
