...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`franchise-portal:${ip}`, 8, 60_000)) {
  ...
  try {
    ...
    await Promise.resolve({
      route: 'franchise-portal',
      framework: 'Franchise Portal',
...
export async function GET() {
  return NextResponse.json({ status: 'healthy', service: 'Franchise Portal', version: '1.0.0' });
}
