...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`investor-deck-site:${ip}`, 8, 60_000)) {
  ...
  try {
    ...
    await Promise.resolve({
      route: 'investor-deck-site',
      framework: 'Investor Deck Site',
...
export async function GET() {
  return NextResponse.json({ status: 'healthy', service: 'Investor Deck Site', version: '1.0.0' });
}
