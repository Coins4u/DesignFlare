...
export async function POST(request: NextRequest) {
  ...
  if (!rateLimit(`hospitality-booking:${ip}`, 8, 60_000)) {
  ...
  try {
    ...
    await Promise.resolve({
      route: 'hospitality-booking',
      framework: 'Hospitality Booking',
...
export async function GET() {
  return NextResponse.json({ status: 'healthy', service: 'Hospitality Booking', version: '1.0.0' });
}
