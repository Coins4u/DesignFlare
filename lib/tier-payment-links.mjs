 ...
 * Keep in sync with lib/tier-payment-links.ts
 */
...
export function isKnownTier(tierName) {
  if (typeof tierName !== 'string' || !tierName.trim()) return false;
  return TIER_NAMES.includes(tierName.trim());
}

/** @deprecated Bank transfer no longer uses payment links. */
export function getPaymentLinkForTier(_tierName) {
  return null;
}
 ...
 */
export function buildCustomerOrderConfirmationEmail({ fullName, tierName, price }) {
